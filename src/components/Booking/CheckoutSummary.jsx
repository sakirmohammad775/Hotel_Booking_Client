...
const CheckoutSummary = () => {
  ...
  // Grab data from state, or use defaults if someone navigates directly to /checkout
  const { basePrice = 80, totalPrice = 80, rooms = 1 } = location.state || {};

  // Dynamic Math (matching your screenshot logic)
  const accommodationTax = (totalPrice * 0.20).toFixed(2); // 20%
  const feesAmount = (totalPrice * 0.45).toFixed(2);       // ~45% matching your image
  const feesTax = (feesAmount * 0.12).toFixed(2);          // 12% on fees
  const finalTotal = (parseFloat(totalPrice) + parseFloat(accommodationTax) + parseFloat(feesAmount) + parseFloat(feesTax)).toFixed(2);
